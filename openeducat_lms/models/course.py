...
class OpCourse(models.Model):
    ...
    suggested_course_ids = fields.Many2many(
        'op.course', 'course_suggested_course_rel', 'course_id', 'suggested_course_id', string='Courses')
    ...
    visibility = fields.Selection([
        ...
        ('invited_user', 'Only Invited User')], 'Visibility Policy', default="public")
    ...
    enrollment_user_ids = fields.Many2many('res.users',
                                           ...
                                           string='Enrolled Users', store=True)

    invited_users_ids = fields.Many2many(
        'res.users', 'course_invited_user_rel', 'plan_id', 'user_id',
        'Invited Users')

    type = fields.Selection([('free', 'Free'),
                             ('paid', 'paid')], 'Course', default='free')
    ...
    total_time = fields.Float(
        'Total Time (HH:MM)', compute='_compute_total_time')
    display_time = fields.Char('Display Time', compute='_compute_display_time')
    ...
    def _compute_graph_data(self):
        for course in self:
            ...
            enrolled_users = self.env[
                'op.course.enrollment'].search_count([('course_id', '=', course.id)])

            course = self.env['op.course'].browse([course.id])
            if course.confirm_date:
                confirm_date = datetime.strptime(
                    course.confirm_date, '%Y-%m-%d').date()
            ...
            course_to_begin = self.env['op.course.enrollment'].search_count(
                [('course_id', '=', course.id), ('state', '=', 'draft')])

            course_in_progress = self.env['op.course.enrollment'].search_count(
                [('course_id', '=', course.id), ('state', '=', 'in_progress')])

            course_completed = self.env['op.course.enrollment'].search_count(
        ...
        for course in self:
            for d in range(1, (fields.date.today().day) + 1):
                ...
                count = self.env['op.course.enrollment'].search_count(
                    [('course_id', '=', course.id), ('enrollment_date', '>=', start_date),
...
class OpCourseSection(models.Model):
    ...
    course_id = fields.Many2one('op.course', 'Course', required=True,
                                ondelete="cascade")
    section_material_ids = fields.One2many(
        'op.course.material', 'section_id', 'Section Materials')
    total_time = fields.Float(
        'Total Time (HH:MM)', compute='_compute_total_time')
    display_time = fields.Char('Display Time', compute='_compute_display_time')
...
class OpCourseMaterial(models.Model):
    ...
    sequence = fields.Integer('Sequence', required=True)
    section_id = fields.Many2one('op.course.section', 'Section', required=True,
                                 ondelete="cascade")
    course_id = fields.Many2one(
        related='section_id.course_id', string='Course')
