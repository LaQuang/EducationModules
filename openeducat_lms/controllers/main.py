...
class OpenEduCatLms(http.Controller):
    ...
    #stem translate
    @http.route(
        '''/course/material_version/<model("op.material.version", "[('datas', '!=', False)]"):material_version>/pdf_content''',
        type='http', auth="public", website=True)
    def material_version_get_pdf_content(self, material_version):
        response = werkzeug.wrappers.Response()
        response.data = material_version.datas and material_version.datas.decode(
            'base64') or ''
        response.mimetype = 'application/pdf'
        return response

    @http.route(
        '''/course/material/<model("op.material", "[('text', '!=', False), ('material_type', '=', 'text')]"):material>/text_content''',
        type='http', auth="public", website=True)
    def material_get_text_content(self, material):
        response = werkzeug.wrappers.Response()
        response.data = material.text
    ...
    #stem translate
    @http.route('/material_versions/embed/<int:material_version_id>', type='http',
                auth='public', website=True)
    def material_versions_embed(self, material_version_id, page="1", **kw):
        try:
            material_version = request.env['op.material.version'].browse(material_version_id)
            return request.render('openeducat_lms.embed_material_version',
                                  {'version': material_version})
        except AccessError:
            material_version = request.env['op.material.version'].sudo().browse(material_version_id)
            return request.render('openeducat_lms.embed_material_version_forbidden',
                                  {'version': material_version})
    #stem translate
